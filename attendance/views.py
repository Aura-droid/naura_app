...
def web_manifest(request):
    manifest = {
        "id": "/?app=md",
        "name": "Mwandet Digital",
        "short_name": "MD Portal",
...
def service_worker(request):
    offline_url = "/offline/"
    icon_192 = f"{settings.STATIC_URL}assets/img/md-app-icon.png"
    icon_512 = f"{settings.STATIC_URL}assets/img/md-app-icon.png"
    js = f"""
...
def export_tod_pdf(request, report_id):
    ...
    logo_path = os.path.join(settings.BASE_DIR, 'static', 'assets', 'img', 'mwandeticon.jpg')
    context = {'report': report, 'school_name': 'Mwandet Digital', 'logo_path': logo_path, 'user': request.user}
...
def export_weekly_tod_summary(request):
    ...
    context = {'reports': reports, 'start_date': start, 'end_date': end, 'school_name': 'MWANDET DIGITAL', 'logo_path': logo_path, 'user': request.user}
